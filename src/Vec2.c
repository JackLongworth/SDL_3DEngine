...
Vec2 Vec2_Scaled(const Vec2 *v, const float scalar)
{
    Vec2 result;
    result.x = v->x * scalar;
    result.y = v->y * scalar;
...
{
    Vec2 result;
    result.x = lhs->x * rhs->elements[0][0] + lhs->y * rhs->elements[1][0];
    result.y = lhs->x * rhs->elements[0][1] + lhs->y * rhs->elements[1][1];
    return result;
}

void Vec2_ApplyMat2(Vec2 *lhs, const struct Mat2 *rhs)
{
    *lhs = Vec2_TimesMat2(lhs, rhs);
}